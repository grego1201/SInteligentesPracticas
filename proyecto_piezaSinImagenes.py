...
class puzzle(tk.Frame):
	...
	def __init__(self,image_o,image_p, ancho, alto):
		...
		if self.comprobarImagenes():
			...
			self.after(5000,self.crearImagen) #crea una imagen del puzzle para probar el metodo
		else:
	...
	def creartableroOriginal(self):
		...
		for x in xrange(self.ancho):
			for y in xrange(self.alto):
				...
				self.tableroOriginal.append(self.nuevaPieza(ids,(x,y))) #almacena la id y la posicion x,y en la matriz
				...
				self.tableroPuzzle.append(self.nuevaPieza(None,(x,y)))
				ids+=1
			...
			for y in range(0,len(self.tableroPuzzle)):
					if self.listaImg_Original[x] == self.listaImg_Puzzle[y]:
						...
						if x == 0: #establece el pivote en el tablero puzzle
							...
							self.pivote=y						
						break
						#self.listaImg_Original.pop()#este no se vacia para poder generar la imagen luego (en self.imagenes hay imagenes en el formato con el que se visualiza)
						self.listaImg_Puzzle.pop() 

		#elf.listaImg_Original=[]
		self.listaImg_Puzzle=[]
	...
	def mostrar(self):
		...
		for x in xrange(self.ancho):
			for y in xrange(self.alto):
				...
				image = self.imagenes[self.tableroPuzzle[index]['id']]
	...
	def mover(self):
		...
		if self.movimientosValidos(self.tableroPuzzle[self.pivote]['pos_o'],movimiento):
			...
			aux=self.pintar(x, y, self.imagenes[ self.tableroPuzzle[self.pivote]['id']])

			x = int(self.tableroPuzzle[indice2]['pos_o'][0]) * self.piece_width
			y = int(self.tableroPuzzle[indice2]['pos_o'][1]) * self.piece_height
			aux=self.pintar(x, y, self.imagenes[self.tableroPuzzle[indice2]['id']])
